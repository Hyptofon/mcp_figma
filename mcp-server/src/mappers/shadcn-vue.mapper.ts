// ─── shadcn-vue Component Mapper ───

import type { IRComponentType } from '../types/ir.types.js';

export interface ShadcnVueMapping {
...
const SHADCN_VUE_MAP: Partial<Record<IRComponentType, ShadcnVueMapping>> = {
    button: {
    ...
    select: {
        ...
        imports: ['Select', 'SelectContent', 'SelectGroup', 'SelectItem', 'SelectTrigger', 'SelectValue'],
        wrapperTag: 'Select',
        childrenSlot: true,
    },
    checkbox: {
...
export function getShadcnVueMapping(type: IRComponentType): ShadcnVueMapping | null {
    return SHADCN_VUE_MAP[type] ?? null;
}

export function getAllShadcnVueImports(types: IRComponentType[]): Map<string, string[]> {
    const importMap = new Map<string, string[]>();

    for (const type of types) {
        const mapping = SHADCN_VUE_MAP[type];
        if (mapping) {
